  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-content-primary">{t('orders.newOrder')}</h1>
      </div>

      <form
        noValidate
        onSubmit={handleSubmit((data) => mutation.mutate(data))} className="flex flex-col gap-6">
        ...
        <div className="flex justify-end gap-3">
          <Button variant="secondary" type="button" onClick={() => navigate('/')}>{t('common.cancel')}</Button>
          <Button type="submit" loading={mutation.isPending}>{t('orders.createOrder')}</Button>
        </div>
      </form>
    </div>
  )
